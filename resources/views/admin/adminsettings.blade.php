...
<html lang="en">
    ...
    <body class="bg-gray-100">
        ...
        <div class="bg-white my-10 mx-10 px-20 py-10 shadow-lg rounded-lg">
            ...
            <h1 class="text-2xl font-semibold mb-10 text-center">Admin Settings</h1>
            @if(session('success'))
            <div class="bg-green-200 text-green-700 -mt-5 p-4 mb-4 rounded text-center">
                {{ session('success') }}
            </div>
            @endif
            <form action="{{ route('updatepassword') }}" method="post" class="text-black">
                @csrf
                @method('PUT')
                <div class="mb-2.5">
                    <label for="name" class="block text-sm font-medium text-gray-600">Nama</label>
                    <input type="text" name="name" id="name" value="{{ $user->nama }}" class="mt-1 p-2 w-full border rounded-md">
                </div>
                <div class="mb-2.5">
                    <label for="email" class="block text-sm font-medium text-gray-600">Email</label>
                    <input type="email" name="email" id="email" value="{{ $user->email }}" class="mt-1 p-2 w-full border rounded-md" disabled>
                </div>
                <div class="mb-2.5">
                    <label for="old_password" class="block text-sm font-medium text-gray-600">Password Lama</label>
                    <input type="password" name="old_password" id="old_password" class="mt-1 p-2 w-full border rounded-md">
                </div>
                <div class="mb-2.5">
                ...
                <div class="flex items-center justify-between mt-6">
                    <button type="submit" class="bg-blue-700 hover:bg-blue-800 transition ease-in-out duration-100 text-white px-4 py-2 rounded-md">Simpan</button>
                </div>
            </form>
        </div>
    </body>
